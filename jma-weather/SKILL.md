...
| コード | 地域 | コード | 地域 |
| --- | --- | --- | --- |
| 016000 | 札幌 (石狩・空知・後志地方) | 130000 | 東京 |
| 040000 | 仙台 (宮城県) | 270000 | 大阪 |
| 230000 | 名古屋 (愛知県) | 340000 | 広島 |
| 460100 | 鹿児島 (奄美地方除く) | 471000 | 那覇 (沖縄本島地方) |

`offices` コードは市区町村ではなく地方予報単位の名前になっている(例: 札幌周辺は `016000`「石狩・空知・後志地方」)。表にない地域は `area.json` の `name` で検索して対応するコードを探す。
...
- 沿岸部の `areas` には `winds` (風) と `waves` (波高) が入ることがある(内陸の地域にはない)。使わなくてもよいが、存在していても異常ではない。
