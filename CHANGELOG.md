...
- 新スキル `amedas-weather`: 気象庁アメダスの最新観測値(気温・降水量・風・湿度など)を全国約1,300か所から取得(2026-09-19実測)

## [0.2.0] - 2026-09-12

### Added
- calil-books: 東京の既定照会先として都立+23区の区立図書館のsystemid一覧を追加(2026-09-14にカーリルAPIで実測確認)
- 新スキル4個を追加(2026-09-13): `eew-monitor`(緊急地震速報の発表状況と直近の地震・津波情報。wolfxとP2P地震情報の非公式中継APIを利用し、公式確認はbosai-alertへ誘導)、`estat-stats`(e-Stat API v3による統計表検索・メタ情報・データ取得。無料appId前提)、`garbage-day`(5374形式の公開CSVによるごみ収集日照会。金沢・美唄・松前・笠間・上砂川・大分を実測)、`koyobun-check`(文化審議会建議「公用文作成の考え方」ベースの公用文校閲チェックリスト。ネットワーク不要)
